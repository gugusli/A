...
<div class="form-page">
    ...
    <form method="POST" action="/preceptora/previa/editar" class="form-card">
        ...
        <div class="form-group">
            ...
            <select name="alumno_id" required>
                ...
                <option value="<?= (int)$a['id'] ?>" <?= $a['id'] == $previa['alumno_id'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($a['nombre'], ENT_QUOTES, 'UTF-8') ?> (<?= htmlspecialchars($a['curso'], ENT_QUOTES, 'UTF-8') ?>)
                </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="form-group">
            <label>Horario</label>
            <input type="time" name="horario" value="<?= htmlspecialchars(substr($previa['horario'],0,5), ENT_QUOTES, 'UTF-8') ?>" required>
        </div>
        <div class="form-group">
            <label>Aula</label>
            <input type="text" name="aula" value="<?= htmlspecialchars($previa['aula'], ENT_QUOTES, 'UTF-8') ?>" maxlength="20" required>
        </div>
