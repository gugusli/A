...
class Previa {
    ...
    public static function create(array $data): int {
        $stmt = DB::get()->prepare(
            'INSERT INTO previas (alumno_id, materia_id, fecha, horario, aula)
             VALUES (:alumno_id, :materia_id, :fecha, :horario, :aula) RETURNING id'
        );
        $stmt->execute([
    ...
    public static function update(int $id, array $data): void {
        $stmt = DB::get()->prepare(
            'UPDATE previas SET alumno_id=:alumno_id, materia_id=:materia_id,
             fecha=:fecha, horario=:horario, aula=:aula WHERE id=:id'
        );
        $stmt->execute([
            ':alumno_id'  => $data['alumno_id'],
            ':materia_id' => $data['materia_id'],
